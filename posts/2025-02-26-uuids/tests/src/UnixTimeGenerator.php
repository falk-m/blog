...
class UnixTimeGenerator
{
    ...
    {
        $time = (new DateTimeImmutable('now'))->format('Uv');
        ...
        if ($this->intSize < 8) {
            throw new Exception('Please use Ramsey\Uuid\Generator\UnixTimeGenerator');
        }
    ...
    {
        if (self::$seed === null) {
            $seed = $this->getRandomBytes(16);
            self::$seed = $seed;
        } else {
            $seed = $this->getRandomBytes(10);
        }
